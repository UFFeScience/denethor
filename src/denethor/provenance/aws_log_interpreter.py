...
def process_and_save_logs(
    ...
    if not workflow_db:
        raise ValueError(
            f"Workflow {workflow_dict['workflow_name']} not found in Database!"
        )
    ...
    configurations_db = provider_configuration_repo.get_by_provider_id(
        provider_db.provider_id
    )
    if not configurations_db:
        raise ValueError(
            f"Provider Configuration for {provider_db} not found in Database!"
        )
    ...
    for request_id, logs in logs_by_request.items():
        ...
        service_execution = parser.parse_execution_logs(
            request_id, activity_name, logs, statistics_dict
        )
        ...
        # Find and validade the proper configuration by the memory size recovered from the log
        memory_from_log = service_execution.memory_size
        if not memory_from_log:
            raise ValueError(
                f"Memory size not found in log for {activity_name} | RequestId: {request_id}"
            )
        
        if memory_from_log != memory:
            raise ValueError(
                f"Memory size in log mismatch for {activity_name} | RequestId: {request_id} | Expected: {memory} | Found: {memory_from_log}"
            )
        
        for config in configurations_db:
            if config.memory_mb == memory_from_log:
                service_execution.provider_configuration = config
                break
        
        if not service_execution.provider_configuration:
            raise ValueError(
                f"Provider Configuration not found for {provider_db} | Activity: {activity_db} | Memory: {memory_from_log}"
            )

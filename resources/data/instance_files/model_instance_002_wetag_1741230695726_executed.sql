...
-----------'wetag_1741230695726'
-----------Generated at 2025-03-30 18:59:55
...
SELECT 
	(SELECT count(distinct st.task_id) AS _tasks_count
		...
		WHERE we.execution_tag in ('wetag_1741230695726')
	),
...
-----------'wetag_1741230695726'
-----------Generated at 2025-03-30 18:59:56
...
produced_file AS (
	...
	JOIN workflow_execution we ON se.we_id = we.we_id
	WHERE ef.transfer_type = 'produced' AND we.execution_tag in ('wetag_1741230695726')
	GROUP BY ef.file_id
),
consumed_files AS (
	SELECT
	...
	JOIN service_execution se ON se.se_id = ef.se_id
	JOIN workflow_execution we ON se.we_id = we.we_id
	WHERE ef.transfer_type = 'consumed' AND we.execution_tag in ('wetag_1741230695726')
	GROUP BY ef.file_id
)
SELECT 

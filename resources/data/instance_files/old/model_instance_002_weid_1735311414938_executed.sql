...
-----------'weid_1735311414938'
-----------Generated at 2024-12-27 14:23:12
...
SELECT 
	(SELECT count(distinct se.task_id) AS _tasks_count
		...
		WHERE se.workflow_execution_id in ('weid_1735311414938')
	),
...
produced_file AS (
	...
	WHERE ef.transfer_type = 'produced' AND se.workflow_execution_id in ('weid_1735311414938')
	GROUP BY ef.file_id
),
consumed_files AS (
	SELECT
	...
	JOIN file f ON f.file_id = ef.file_id
	JOIN service_execution se ON se.se_id = ef.se_id
	WHERE ef.transfer_type = 'consumed' AND se.workflow_execution_id in ('weid_1735311414938')
	GROUP BY ef.file_id
)
SELECT 

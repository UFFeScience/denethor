...
--dynamic_files
SELECT distinct se.se_id, se.workflow_execution_id, f.file_id, f.file_name, 'dynamic' as file_type
FROM file f
...
UNION
--static_files
SELECT distinct se.se_id, se.workflow_execution_id, f.file_id, f.file_name, 'static' as file_type
FROM file f
JOIN execution_file ef ON ef.file_id = f.file_id
JOIN service_execution se ON se.se_id = ef.se_id
